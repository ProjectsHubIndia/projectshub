const roadmapData = {
  ...
  windsurf: {
    title: "Windsurf",
    desc: "<p>Windsurf is an AI-powered IDE with an agentic flow that can plan and execute multi-step coding tasks across files. It emphasizes a smooth human-in-the-loop coding experience. A direct alternative to Cursor.</p>",
  },
  claude_code: {
    title: "Claude Code",
...
function openDrawer(tagId) {
  const data = roadmapData[tagId];
  if (!data) return;

  const ev = window.event;
  if (ev && ev.target) {
    const timelineItem = ev.target.closest(".timeline-item");
    if (timelineItem) {
      const items = Array.from(document.querySelectorAll(".timeline-item"));
      const index = items.indexOf(timelineItem);
      ...
      .map(
        (item) => `
                    <a href="#" class="resource-link">
                        <span class="resource-type ${item.type}">${item.label}</span>
  ...
  // Populate Sections
  document.getElementById("freeList").innerHTML = generateLinks(data.free);
  document.getElementById("tutorList").innerHTML = generateLinks(data.tutor);
  document.getElementById("premiumList").innerHTML = generateLinks(
    data.premium,
  );

  // Populate topic image
  // Future: replace data.image with real image URL per topic in roadmapData
  const drawerImg = document.getElementById("drawerImage");
  const drawerLabel = document.getElementById("drawerImageLabel");
  const imgContainer = document.getElementById("drawerImageContainer");

  if (drawerImg) {
    // Use data.image if defined, otherwise generate a themed dummy placeholder
    const encodedTitle = encodeURIComponent(data.title);
    const placeholderUrl =
      data.image ||
      `https://placehold.co/500x200/0f111a/00f0ff?text=${encodedTitle}&font=montserrat`;

    drawerImg.src = placeholderUrl;
    drawerImg.alt = data.title;
    if (drawerLabel) drawerLabel.textContent = "📖  " + data.title;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  timelineItems.forEach((item, index) => {
    ...
    const heading = item.querySelector("h3").innerText;
