...
| 🟠 **HIGH** | `projectshub/settings.py:27-29, 46-57` | Overly Permissive Wildcard `*` in Default `ALLOWED_HOSTS` | `ALLOWED_HOSTS` previously defaulted to `['*']` without domain sanitization. | Wildcard `*` allows Host Header Poisoning. However, removing platform domains causes `DisallowedHost` (HTTP 400). | Configure base trusted domains (`projectshub.co.in`, `projectshub-production.up.railway.app`, `.railway.app`, `.vercel.app`), sanitize environment variables, and automatically merge platform domains. |
